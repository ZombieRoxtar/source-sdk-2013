...
/* BUGBUG: No such sound. Should this there be?
#ifndef HL2MP
...
//-----------------------------------------------------------------------------
// Purpose: Quickly switch to the named weapon, or back to previous item
//-----------------------------------------------------------------------------
void CC_Player_MasterSwap( const char* pweap)
{
	CBasePlayer* pPlayer = ToBasePlayer(UTIL_GetCommandClient());
	...
	if (pPlayer)
	{
		...
		if (pWeapon)
		{
			...
			if (!Q_stricmp(strWeaponName, pweap))
			{
...
{
	CC_Player_MasterSwap("weapon_physcannon");
}
...
{
	CC_Player_MasterSwap("weapon_bugbait");
}
static ConCommand bugswap("bug_swap", CC_Player_BugBaitSwap, "Automatically swaps the current weapon for the bug bait and back again." );
#endif
...
{
	CC_Player_MasterSwap("weapon_portalgun");
}
