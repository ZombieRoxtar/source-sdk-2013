...
static ConVar	hud_portalquickinfo( "hud_portalquickinfo", "1", FCVAR_ARCHIVE );
static ConVar	hud_portalquickinfo_swap( "hud_portalquickinfo_swap", "0", FCVAR_ARCHIVE );
...
class CHUDPortalQuickInfo : public CHudElement, public vgui::Panel
{
	DECLARE_CLASS_SIMPLE(CHUDPortalQuickInfo, vgui::Panel );
public:
	CHUDPortalQuickInfo( const char *pElementName );
	void Init( void );
	void VidInit( void );
...
DECLARE_HUDELEMENT(CHUDPortalQuickInfo);

CHUDPortalQuickInfo::CHUDPortalQuickInfo( const char *pElementName ) :
	CHudElement( pElementName ), BaseClass( NULL, "HUDPortalQuickInfo" )
{
...
void CHUDPortalQuickInfo::ApplySchemeSettings( IScheme *scheme )
{
...
void CHUDPortalQuickInfo::Init( void )
{
	m_flLastEventTime   = 0.0f;
}

void CHUDPortalQuickInfo::VidInit( void )
{
	Init();
...
void CHUDPortalQuickInfo::DrawWarning( int x, int y, CHudTexture *icon, float &time )
{
...
//-----------------------------------------------------------------------------
bool CHUDPortalQuickInfo::ShouldDraw( void )
{
...
void CHUDPortalQuickInfo::Paint()
{
	...
	if ( !hud_portalquickinfo.GetInt() || !pPortalgun || ( !pPortalgun->CanFirePortal1() && !pPortalgun->CanFirePortal2() ) )
	{
	...
	if ( !hud_portalquickinfo_swap.GetBool() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHUDPortalQuickInfo::UpdateEventTime( void )
{
...
//-----------------------------------------------------------------------------
bool CHUDPortalQuickInfo::EventTimeElapsed( void )
{
